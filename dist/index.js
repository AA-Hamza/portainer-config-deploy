...
class PortainerApi {
    ...
    async getAllConfigs() {
        const { data } = await this.axiosInstance.get(`/endpoints/${this.endpointId}/docker/configs`);
        return data;
    }
    async createConfig(name, data, isBase64Encoded = false) {
    ...
    async getConfig(id) {
        const { data } = await this.axiosInstance.get(`/endpoints/${this.endpointId}/docker/configs/${id}`);
        return data;
    }
    async cloneConfig(id, newName) {
...
function getConfigContent(relativePath) {
    ...
    const configContent = fs_1.default.readFileSync(configFullPath, 'utf8');
    if (!configContent) {
        throw new Error(`Could not find config file: ${configFullPath}`);
    }
    return configContent;
}
async function deployConfig({ portainerHost, username, password, endpointId, configName, configPath, rejectUnauthorized }) {
    const portainerApi = new api_1.PortainerApi(portainerHost, endpointId, rejectUnauthorized);
...
const core = __importStar(__nccwpck_require__(2186));
const axios_1 = __importDefault(__nccwpck_require__(8757));
const deployConfig_1 = __nccwpck_require__(2885);
function getInputs() {
    ...
    const configPath = core.getInput('config-definition', {
        required: false
    });
...
async function run() {
    try {
        ...
        await (0, deployConfig_1.deployConfig)(userInputs);
